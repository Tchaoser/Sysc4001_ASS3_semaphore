    ...
    for (int ta_id = 1; ta_id <= NUM_TAS; ta_id++) {
        ...
        if (pid < 0) {
            ...
            while (cycle_count < NUM_CYCLES) {
                ...
                // Modified protocol for semaphore acquisition:
                // Try to get left_sem, then try right_sem with sem_trywait.
                while (1) {
                    // Wait on left_sem
                    sem_wait(&shm->sem[left_sem]);

                    // Try to acquire right_sem non-blockingly
                    if (sem_trywait(&shm->sem[right_sem]) == 0) {
                        // Acquired both successfully
                        break;
                    } else {
                        // Right semaphore not available
                        // Release the left one and wait before trying again
                        sem_post(&shm->sem[left_sem]);
                        usleep(100000); // wait 0.1 sec before retry
                    }
                }
